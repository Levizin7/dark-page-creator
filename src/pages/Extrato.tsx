...
import { ArrowLeft, ArrowUpRight, ArrowDownLeft, Copy, X } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
import { useNavigate } from "react-router-dom";
import { useTransactions, Transaction, formatTxDate } from "@/contexts/TransactionContext";
    ...
    <div className="min-h-screen bg-background max-w-md mx-auto relative pb-24">
        ...
        {(["all", "income", "expense"] as const).map((f) => (
          <motion.button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-body font-medium border transition-all ${
              filter === f ? "bg-accent/15 border-accent/30 text-accent" : "bg-card border-border text-muted-foreground"
            }`}
          >
      ...
      <div className="px-5 space-y-2.5">
        {loading ? (
          ...
          <p className="text-center py-16 text-muted-foreground font-body text-sm">Nenhuma transação encontrada</p>
        ) : (
          filtered.map((tx, i) => (
            <motion.div
              key={tx.id}
            ...
            >
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${tx.type === "income" ? "bg-success/15" : "bg-destructive/15"}`}>
                {tx.type === "income" ? <ArrowDownLeft size={18} className="text-success" /> : <ArrowUpRight size={18} className="text-destructive" />}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-body font-medium text-foreground truncate">{tx.title}</p>
                <p className="text-[10px] text-muted-foreground font-body">{formatTxDate(tx.created_at)}</p>
              </div>
      ...
      <AnimatePresence>
        {selectedTx && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setSelectedTx(null)} className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50" />
            <motion.div
            ...
            >
              <div className="bg-card rounded-t-3xl border-t border-border p-5">
                ...
                <div className="flex items-center justify-between mb-5">
                  ...
                  <motion.button whileTap={{ scale: 0.9 }} onClick={() => setSelectedTx(null)}><X size={20} className="text-muted-foreground" /></motion.button>
                </div>
                ...
                <div className="text-center mb-6">
                  <div className={`w-14 h-14 rounded-2xl mx-auto mb-3 flex items-center justify-center ${selectedTx.type === "income" ? "bg-success/15" : "bg-destructive/15"}`}>
                    {selectedTx.type === "income" ? <ArrowDownLeft size={24} className="text-success" /> : <ArrowUpRight size={24} className="text-destructive" />}
                  </div>
                ...
                <div className="space-y-3 bg-muted/50 rounded-2xl p-4">
                  <DetailRow label="Data e hora" value={formatTxDate(selectedTx.created_at)} />
                  ...
                  <div className="flex items-center justify-between pt-1">
                    ...
                    <motion.button whileTap={{ scale: 0.9 }} onClick={() => copyCode(selectedTx.code)} className="p-2 rounded-lg bg-accent/10 hover:bg-accent/20 transition-colors">
                      <Copy size={14} className="text-accent" />
                    </motion.button>
                  </div>
                </div>
                <p className="text-[9px] text-muted-foreground/40 font-body text-center mt-4">ID: {selectedTx.id}</p>
              </div>
            </motion.div>
          </>
        )}
